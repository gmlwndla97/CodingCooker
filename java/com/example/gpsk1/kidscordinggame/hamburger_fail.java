...
public class hamburger_fail extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hamburger_fail);
    ...
    public void onClicked(View args) {
        ...
        if(((ImageButton)args).getId()==R.id.restart){
            if(number==1){
                Intent intent=new Intent(hamburger_fail.this, hamburgerhint1.class);
                startActivity(intent);
            }
            if(number==2){
                Intent intent=new Intent(hamburger_fail.this, hamburgerhint2.class);
                startActivity(intent);
            }
            if(number==3){
                Intent intent=new Intent(hamburger_fail.this, hamburgerhint3.class);
                startActivity(intent);
            }
            finish();
        }
        else if(((ImageButton)args).getId()==R.id.stop){
            Intent intent=new Intent(hamburger_fail.this, menu.class);
            startActivity(intent);
            finish();
        }
    }
